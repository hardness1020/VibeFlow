...
- Allows orchestrator/register commands on main (so users can create work items)
...
try:
    ...
    if current_branch in ("main", "master"):
        ...
        print(json.dumps({
            "decision": "block",
            "reason": f"[VibeFlow] Work blocked on '{current_branch}'. Switch to an active work item branch ({active_list}) or register a new one: /vibeflow-orchestrator register \"<description>\" <ID> <track>"
        }))
        sys.exit(0)
